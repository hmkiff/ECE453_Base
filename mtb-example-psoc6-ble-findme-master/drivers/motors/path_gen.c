...
int path_traveled = 0;
//struct POSE * est_pose = {0,0,0};
struct POSE * target_pose = {0,0,0};
...
// set up a rate basis to keep it on schedule.
    int index;
    int waypoint_index = (index % QLENGTH);
    int updateDelay = 100;  // 100 ms 10Hz
    struct POSE est_pose;
    while(!path_is_complete){
        target_pose = newPose();
        est_pose = IMU_read();
        path_segment_spec = createWaypointPath(est_pose);
